...
uses
    ...
    Vcl.ComCtrls, Vcl.Shell.ShellCtrls, Vcl.FileCtrl, XPMan, Vcl.StdCtrls,
    ShellApi, Vcl.ExtCtrls, Vcl.Menus, ActiveX, ShlObj, ComObj, Vcl.Grids;
...
type
  ...
  public
    ...
    abort : boolean;  // do not change string or Terminate not works
...
const  // Data regarding the method of determining access
...
{ easiest and most modern way to get a file's last access time is by using
  the TFile record from the System.IOUtils unit.}
...
{ identifies the type of an open file or I/O handle, such as a disk file,
  pipe, or character device. }
function MrsGetFileType(const strFilename: string): string;
var
  FileInfo: TSHFileInfo;
begin
  // To fill a memory area byte-by-byte with a specified value
  FillChar(FileInfo, SizeOf(FileInfo), #0);
  // Retrieving information about objects in the file system (files, folders, virtual shell objects)
...
var
  SecDescr: PSecurityDescriptor;   // manages an object's owner, primary group
...
begin
  GetFileOwner := False;
  GetMem(SecDescr, 1024);          // SecurityDescriptor
  GetMem(OwnerSID, SizeOf(PSID));  // sid
  GetMem(OwnerName, 1024);         // user
  GetMem(DomainName, 1024);        // domain
  try
    // Information about security descriptors
    if not GetFileSecurity(PChar(FileName),
      OWNER_SECURITY_INFORMATION,
      SecDescr, 1024, SizeNeeded) then
      Exit;

    // Result returned as a PSID (Security Identifier)
    ...
    // determines the account name and domain for a given security identifier (SID)
...
begin
  ...
  begin
    // enable the pop-up menu for the list box.
    Popup.Items[i].Enabled := true;
  end;
end;
...
begin
  for i := 0 to Popup.Items.Count - 1 do
  begin
    // disable the pop-up menu for the list box.
    Popup.Items[i].Enabled := false;
  end;
end;

{ refers to the structured resource data (Version Information) embedded in
...
var
  ...
  { helper function used when reading a file's version information
    (such as an EXE or DLL file). It specifically performs a bitwise
    AND operation to check if a specific flag is set inside the
    dwFileFlags field of a Windows VS_FIXEDFILEINFO structure. }
  ...
  // Reading file and product versions from an executable file or DLL
  ...
  // wrappers to fetch a single scalar value from a query
...
{ to programmatically extend NTFS file permissions
  (Access Control Lists or ACLs) via the Win32 API }
...
{ function typically uses the Windows API to verify if the current user
  has specific permissions (like read, write, or modify) for a file
  or directory. }
...
begin
  ...
  // Retrieve security information for a file or directory.
  ...
  // compare with the security owner
  ...
  try
    ...
    // function used to allow a thread to impersonate the security context of the process itself
    if not ImpersonateSelf(SecurityImpersonation) then
      Exit;
    try
      // Retrieve the access token of a specific thread.
      ...
      try
        ...
        // Access rights for an object- or file-specific access mask
        MapGenericMask(AccessMask, GenericFileMapping);
        // user's access token holds the necessary security privileges
...
// Native properties dialog box for multiple files
function SHMultiFileProperties(pDataObj: IDataObject; Flag: DWORD): HRESULT;
  stdcall; external 'shell32.dll';
{$R-}

{ community-driven wrapper function often used to wrap a list of file
  paths into a standard COM IDataObject. }
...
begin
  ...
  // Retrieve the Windows shell interface
  OleCheck(SHGetMalloc(Malloc));
  {  initialize the root of the Windows Shell Namespace (the Desktop) and
    catch any COM interface errors. }
  OleCheck(SHGetDesktopFolder(Root));
  // dynamic arrays of pointers to Windows shell objects (PIDLs)
  p := AllocMem(SizeOf(PItemIDList) * FileCount);
  try
    for i := 0 to FileCount - 1 do
      try
        // Check whether the path or the file exists.
        if not (System.SysUtils.DirectoryExists(Files[i]) or FileExists(Files[i])) then Continue;
        { to generate a Windows-internal memory address (a PItemIDList or PIDL)
          from a file or folder path. }
    ...
    { Request user interface objects such as a context menu (IContextMenu)
      or a data object (IDataObject). }
    ...
    for i := 0 to FileCount - 1 do
    begin
      // to process multiple file items or folders at once.
...
// execute windows properties dialog
...
// Special and very precise determination of the size of a file
...
// Determine the path with or without a backslash.
...
// Search for a specific file in a folder.
...
var
  ...
  hFile : THandle;
begin
  // Check whether a directory path ends with a backslash (\) before appending further paths.
  if AnsiLastChar(RootFolder)^ <> '\' then
    RootFolder := RootFolder + '\';
  if Recurse then
  begin
    // "*.*" for all files, or change the file format, e.g., "*.exe"
    ...
    try
      ...
      //windows.FindClose(hFile); // old VCL for new VCL = WinApi.windows.FindClose(hFile);
    end;
  end;
  // Specify the files to search here
  hFile := FindFirstFile(PChar(RootFolder + '*.*'), wfd);
  // if no invalid trade
  if hFile <> INVALID_HANDLE_VALUE then
  try
...
begin
  with Bmp do
  begin
    //Canvas.Brush.Color := clFuchsia;  // background color
    //TransparentColor := clFuchsia;    // clFuchsia for Transparent color
    Width := 16;                        // max width
    Height := 16;                       // max height
    Canvas.Draw(0, 0, Ico);             // draw bitmap
    ...
    // Adjust the size if it does not match.
    Canvas.StretchDraw(Rect(0, 0, WH, WH), Bmp);
    Width := WH;
    Height := WH;
    Transparent :=  True; // clBlack color
  end;
end;

// Get a file's icon using shell32.dll
...
// Draw the identified icons into the list box.
...
begin
  ...
  begin
    ...
    Bmp := TBitmap.Create;
    try
      { Check whether a specific item in an owner-draw control
        (such as TListBox or TComboBox) is currently selected. }
      ...
      // Assign the icon to the item in the list box.
      GetIconFromFileB(Items[Index], Icon, True);
      // convert icon to bitmap
      IcoToBmpA(Icon, Bmp, True);
      // Position the icon within the list box item.
...
// completely remove the file from the disk
...
var
  ...
  ContentPtr: Pointer;
begin
  // variable buffer size for memory
    ...
    try
      ...
      begin
        if Times mod 2 = 0 then
          { Constant representing even bytes (e.g., $00, $55, or $AA),
            while ODD_FILL represents odd bytes (e.g., $FF or $AA). }
          FillMemory(ContentPtr, BUFSIZE, EVEN_FILL)
        else
          FillMemory(ContentPtr, BUFSIZE, ODD_FILL);

        // repositions the current read/write pointer of a stream
        ...
        begin
          ...
          begin
            ...
            if Form1.abort = true then Exit;
          end;

          // Write data from the memory area to the stream.
          Fs.Write(ContentPtr^, BUFSIZE);
          // built-in procedure used to decrement a variable's value
          Dec(N);
          // abort function
          if Form1.abort = true then Exit;
          // update form
        ...
        { calculates the remaining bytes that don't fit perfectly into a
          final buffer of size BUFSIZE }
        N := Size mod BUFSIZE;
        if N > 0 then
          Fs.Write(ContentPtr^, N);

        // forces the operating system to immediately flush any buffered data
        FlushFileBuffers(Fs.Handle);
        // to decrease the integer value (ordinal type such as Integer or Word) by 1.
        Dec(Times);
      end;
    finally
...
// Delete the entire folder and all subfolders.
...
// Removing file attributes and file path separators
function TForm1.DelTreeEx(const Path: string; AbortOnFailure: Boolean;
...
begin
  ...
  try
    ...
    // Determine the complete list of all files.
    ...
    begin
      // abort function
      if stop = 's' then Exit;
      // Insert a slash between the folder path and the file path.
      ...
      // Check whether attributes or separators exist.
      if (Attr <> DWORD(-1)) and ((Attr and FILE_ATTRIBUTE_DIRECTORY) <> 0) then
        // Delete all files and subfiles.
  ...
  begin
    ...
    begin
      ...
      RmDir(LPath);
      {$I+} // Switch on I/O error checking
      Result := IOResult = 0;
    end;
  end;
  Form1.ProgressBar1.Position := 0;
...
begin
  ...
  RadioButton1.Hint := 'This feature shreds bytes in blocks to speed' +#13#10+
                       'up the process.';
  RadioButton2.Hint := 'This function shreds every single bit of a' +#13#10+
                       'byte, which takes longer but is more accurate.';
  RadioButton3.Hint := 'Overwrites the file`s storage space with' +#13#10+
                       'cryptographically random characters (RNG).';
  CheckBox1.Hint := 'This function overwrites the directory' +#13#10+
                    'separator so that the path can no longer be determined.';
  CheckBox2.Hint := 'This function manipulates the file attributes so that' +#13#10+
                    'it does not allow for subsequent comparison.';
  CheckBox3.Hint := 'To use this function, the correct language for the' +#13#10+
                    'permissions must be entered in the code.';
end;

// Get detailed information about the file and pass it to the StringGrid.
...
// Create the horizontal scrollbar for the list box.
...
// Enable the removal of file separators.
...
// Shred random bytes in a file
procedure ShredderFileRandomize(const AFileName: string);
const
  BufferSize = 4096;
...
begin
  ...
  // Obtained write access to file
  Fs := TFileStream.Create(AFileName, fmOpenReadWrite or fmShareExclusive);

  try
    BytesRemaining := Fs.Size;
    Randomize; // Initialize random number generator
    // 1. Pass: Overwrite with random bytes
    Fs.Position := 0;
    while BytesRemaining > 0 do
    begin
      // Write random bytes to the buffer
      for i := 0 to High(Buffer) do
        Buffer[i] := Random(StrToInt(Form1.ComboBox1.Text));

      // determine the remaining bytes in a stream
      if BytesRemaining > BufferSize then
        BytesToWrite := BufferSize
      else
        BytesToWrite := BytesRemaining;

      // write raw data from a variable or buffer into that stream
      Fs.Write(Buffer, BytesToWrite);
      // subtracts the value of the variable BytesToWrite from the variable BytesRemaining
      Dec(BytesRemaining, BytesToWrite);

      // write shred bytes
      Form1.Label1.Caption := IntToStr(Random(BytesToWrite));
      // write last access time
      Form1.Label6.Caption := DateTimeToStr(GetFileLastAccessTime(AFileName));
      // write file size
      Form1.StatusBar1.Panels[3].Text :=  IntToStr(FileSize(AFileName)) + ' bytes';
      // check for generic write access
      Form1.Label8.Caption := (BoolToStr(CheckAccessToFile(GENERIC_READ,
                  AFileName), True) +
                  ': ' +
                  SysErrorMessage(GetLastError));
      ...
      case MyS.dwFileAttributes of
        FILE_ATTRIBUTE_COMPRESSED: Form1.Label10.Caption := ('Compressed');
        FILE_ATTRIBUTE_HIDDEN: Form1.Label10.Caption := ('Hidden');
        FILE_ATTRIBUTE_NORMAL: Form1.Label10.Caption := ('No attributes');
        FILE_ATTRIBUTE_READONLY: Form1.Label10.Caption := ('Read only file');
        FILE_ATTRIBUTE_SYSTEM: Form1.Label10.Caption := ('System file');
        FILE_ATTRIBUTE_TEMPORARY: Form1.Label10.Caption := ('Temporary storage');
        FILE_ATTRIBUTE_ARCHIVE: Form1.Label10.Caption := ('Archive file');
      end;
    ...
    // 2. Pass: Overwrite with zeros (optional but recommended)
    Fs.Position := 0;
...
{$IFDEF MSWINDOWS}    // conditional compilation
{$ELSE} '/' {$ENDIF}  // Preprocessor or compiler directive for conditional compilation
  // Obfuscate file names (optional)
  RenameFile(AFileName, ExtractFileDir(AFileName) + PathDelim + IntToStr(GetTickCount) + '.tmp');
  path := ExtractFileDir(AFileName) + PathDelim + IntToStr(GetTickCount) + '.tmp';

  // Permanently delete file from the storage medium
...
begin
  ...
  if MessageDlg('ATTENTION !'+#13+
    ...
    BEGIN
      ...
      begin
        // randomiz
        ShredderFileRandomize(Edit1.Text);
      end;
    END;
    StatusBar1.SetFocus;
end;
...
begin
    ...
    BEGIN
      Screen.Cursor := crHourGlass;
      StatusBar1.Panels[4].Text := 'Proccessing, please wait..';
      abort := false;
      ...
      begin
         ...
         begin
            ...
            begin
              try
                // Freeze ListBox painting
                ListBox1.Items.BeginUpdate;
                ListBox1.ItemIndex := i;
                StatusBar1.Panels[1].Text := IntToStr(i);
                Application.ProcessMessages;
              finally
                // Re-enable drawing and update the ListBox.
      ...
      begin
         ...
         begin
            if abort = true then Exit;
            ShredderFileRandomize(ListBox1.Items.Strings[i]);
            ProgressBar1.StepIt;
